  ...
  return (
    <>
      <div className="terra-container">
        <p>
          VISA เป็นตัวเลข 13 หรือ 16 หลัก ข้ึนต้นด้วย 4 เช่น 4234123412341 หรือ
          4999888800001111
        </p>
        <p>MASTER เป็นตัวเลข 16 หลัก ขึ้นต้นด้วย 55 เช่น 5534123412341234</p>
        <p>
          AMEX เป็นตัวเลข 15 หลัก ข้ึนต้นด้วย 34 หรือ 37 เช่น 344412341234123
          หรือ 375412341234123
        </p>
        <p>ออกจากโปรแกรมเม่ือรับค่าเป็น -1</p>
        ...
        >
          <input
            ...
            type="text"
          ></input>
          {error.hasOwnProperty(1) && (
            <label className="error-msg">{error['1']}</label>
          )}
          {answer.hasOwnProperty(1) && (
            <label className="answer-msg">{answer['1']}</label>
          )}
              ...
              return (
                <>
                  <div className="terra-wrapper">
                    ...
                    <div />
                    <div>
                      {error.hasOwnProperty(item.id) && (
                        <label className="error-msg flex">
                          {error[item.id]}
                        </label>
                      )}
                    </div>
            ...
            datas.map((item) => (
              <div key={item.id} className="wrapper-modal">
                ...
                {state[item.id] != undefined && (
                  <p className="name-modal">{state[item.id]}</p>
                )}
              </div>
            ))}
        </Modal.Body>
